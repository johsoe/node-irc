...
Client.prototype.connect = function ( retryCount, callback ) { // {{{
    ...
    self.conn.on("error", function(exception) {
        ...
        self.reconnect( retryCount, true );
    });
}; // }}}

Client.prototype.reconnect = function( retryCount, error ) {
    var self = this;
    ...
    // clean up event listeners for next connect()
    self.conn.removeAllListeners();

    // wait for conn to die
    setTimeout( function() {
        util.log( util.format('Trying reconnect() -> %d retries', retryCount) );
