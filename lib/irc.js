...
function Client(server, nick, opt) {
    ...
    self.opt = {
        ...
        password: null,
        userName: 'prebbe',
        realName: 'Preben - nodeJS IRC client',
        port: 6667,
        debug: false,
        joinDelay: 5,
        showErrors: false,
    ...
    self.on('motd', function (motd) {
        // delay channel joins, so authing in Q can get through first
        setTimeout( function() {
            self.opt.channels.forEach(function(channel) {
                self.send.apply(self, ['JOIN'].concat(channel.split(' ')));
            });
        }, self.opt.joinDelay * 1000);
    });
...
Client.prototype.connect = function ( retryCount, callback ) { // {{{
    ...
    self.conn.on("close", function() {
        ...
        setTimeout( function() {
            self.reconnect( retryCount + 1 );
        }, self.opt.retryDelay );
    });
    self.conn.on("error", function(exception) {
        self.emit("netError", exception);
        self.reconnect(  );
    });
}; // }}}

Client.prototype.reconnect = function( retryCount ) {
    var self = this;
    ...
    // wait for conn to die
    setTimeout( function() {
        util.log('Trying reconnect() -> %d retries', retryCount);
        self.reconnect( retryCount );
    }, 30 * 1000 );
};
...
Client.prototype.send = function(command) { // {{{
    ...
    // connection lost
    // this is happening AFTER socket emitting an error we didn't handle
    if( self.conn.readyState != 'open' ) {
        this.reconnect();
        return;
    }
